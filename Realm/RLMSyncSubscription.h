...
/// will fail. If this is set to YES, instead the existing subscription will be
/// updated using the query and options from the new subscription. This only
/// works if the new subscription is for the same type of objects as the
/// existing subscription. Trying to overwrite a subscription with a subscription
/// of a different type will fail.
///
/// The updatedAt and (if timeToLive is used) expiresAt are updated whenever a
/// subscription is overwritten even if nothing else has changed.
...
/**
 ...
 being linked to by the objects in the subscription. If the limit is larger
 than the number of objects which match the query, all objects will be included.
 A limit of zero is treated as unlimited.
 */
